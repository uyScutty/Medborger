...
export default function PublicExamPage() {
  ...
  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center text-gray-400">Indlæser prøve…</div>
    );
  }
  ...
  if (!exam || questions.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center">
        <p className="text-gray-500">Prøven er ikke tilgængelig i øjeblikket.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      <div className="mb-8 text-center">
        <Badge variant="success" className="mb-3">Gratis smagsprøve</Badge>
        <h1 className="text-3xl font-extrabold text-gray-900">{exam.title}</h1>
        <p className="mt-3 text-gray-500">
          {questions.length} spørgsmål · {exam.total_time_minutes} minutter · bestå med {exam.pass_score}+
        </p>
      </div>
      ...
      <Card className="mb-6 space-y-3">
        ...
        {secondLang && (
          <p className="text-xs text-gray-400">
            ...
            {LANGUAGES.find((l) => l.code === secondLang)?.name ?? secondLang}
          </p>
        )}
      </Card>

      <div className="flex flex-col items-center gap-4">
        <Button size="lg" onClick={() => setState("running")} className="w-full sm:w-auto">
          Start prøven →
        </Button>
        <p className="text-sm text-gray-400">
          Ingen konto kræves.{" "}
          <Link href="/register" className="text-brand-red underline">
            Opret konto
          </Link>{" "}
